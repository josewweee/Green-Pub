...
function LogOut(){
	firebase.auth().signOut().then(function() {
    	...
    	// An error happened.
    	window.alert("Un error ha sucedido, por favor comuniquese con lancha para mas informacion");
	});
}
...
// -------------------- FUNCIONES PARA ACTUALZIAR DATOS DEL USUARIO --------------------
function MetPago(){	

	ref.child("payment").update({
  		payment_method: $("#NuevoPago").val()
  	});

  	ref.child("payment").on("value", function(snapshot){
		document.getElementById("Actual_payment_method").innerHTML = snapshot.val().payment_method;
	});
}

function Cambiar_Contraseña(){
	if($("#NuevaContraseña").val() == $("#NuevaContraseña2").val()){
		ref.update({
	  		password: $("#CambiarContraseña").val()
	  	});
	}else{
		window.alert("Asegurate de haber escrito las contraseñas bien");
	}
}
...
function MetCons(){
	...
	ref.child("payment").update({
  		dispatch_method: $("#NuevoCons").val()
  	});

  	ref.child("payment").on("value", function(snapshot){
  		document.getElementById("Actual_dispatch_method").innerHTML = snapshot.val().dispatch_method;
	});
